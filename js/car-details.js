...
document.addEventListener("DOMContentLoaded", function() {
    ...
    ajax.onreadystatechange = function() {
        if (this.readyState == 4) {
            if (this.status == 200) {
                ...
                fuelConsumption.innerHTML = carInfo.FuelConsumption + "l/100km";
                body.innerHTML = carInfo.Body;
                doors.innerHTML = carInfo.Doors;
                price.innerHTML = carInfo.Price.toFixed(2).replace(".", ",") + " zł";

                contactButton.href = "./contact.php?id=" + carInfo.ID + "&brand=" + carInfo.Brand + "&model=" + carInfo.Model;
    ...
    ajax.open("GET", "./api/car-details.php?id=" + (new URL(location.href)).searchParams.get("id"), true);
    ajax.send();
});
...
function deleteCarClick() {
        ...
        if (this.readyState == 4) {
            if (this.status == 200) {
                ...
                if (response) {
                    ...
                    location.href = "./cars.php?type=" + carInfo.Category;
                }
                else {
    ...
    ajax.open("GET", "./api/delete-car.php?id=" + (new URL(location.href)).searchParams.get("id"), false);
    ajax.send();
};